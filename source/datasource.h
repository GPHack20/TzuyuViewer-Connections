...
public:
    CDataSource();
    ~CDataSource();
    typedef enum{ //https://users.cs.northwestern.edu/~agupta/cs340/project2/TCPIP_State_Transition_Diagram.pdf
    ...
    typedef struct{
        ...
        char                localAddr[INET6_ADDRSTRLEN]; //It is neccesary to use the IPv6 lenght to store IPv4 and IPv6
        char                localHost[NI_MAXHOST];
        char                localPort[NI_MAXSERV];

        char                remoteAddr[INET6_ADDRSTRLEN]; //It is neccesary to use the IPv6 lenght to store IPv4 and IPv6
